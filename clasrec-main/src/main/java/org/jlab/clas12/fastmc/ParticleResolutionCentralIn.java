 ...
 * @author gavalian
 */
public class ParticleResolutionCentralIn implements IParticleResolution {
    ...
    /*
          pnorm=p*Tmax/abs(torcur)^M^M
c^M^M                                                                                                                                                                                                 
      thr=thetad*d2r^M^M
      phir=phid*d2r^M^M
c     select which of the 6 functions we want^M^M                                                                                                                                                     
      if(inbend.and.fwd)then^M^M
ckm 14-apr-2005 (fit on all thetas), theta, phi resolutions:^M^M                                                                                                                                      
         sig1p=Tmax/torcur*^M^M
     &        (FoMT1S1+FoMT1S2*thetad+FoMT1S3*thetad*thetad)^M^M
         sig2p=Tmax/torcur*^M^M
     &        (FoMT2S1+FoMT2S2*thetad+FoMT2S3*thetad*thetad)^M^M
ckm   sigma_theta is at 5 degrees(as upper limit)^M^M                                                                                                                                                 
         sig1th=FoInThS1^M^M
         sig2th=FoInThS2^M^M
ckm May 28, 2005: Gail's phi-resolution as function of theta^M^M                                                                                                                                      
ckm         sig1fi=2.*sig1th^M^M                                                                                                                                                                      
ckm         sig2fi=2.*sig2th^M^M                                                                                                                                                                      
^M^M
ckm         sig1fi=FoIOFiS1/SIN(thr)^M^M                                                                                                                                                              
c         print*,'sig1fi,FoIOFiS1,thr',sig1fi,FoIOFiS1,thr*r2d^M^M                                                                                                                                    
ckm         sig2fi=FoA0FiS2+FoA1FiS2*thr*r2d+FoA2FiS2*(thr*r2d)**2!theta in degrees^M^M                                                                                                               
^M^M
ckm May 31, 2005^M^M                                                                                                                                                                                  
         sig1fi=FoA0FiS1/(thr*r2d) + FoA1FiS1 + FoA2FiS1*(thr*r2d)!theta in degrees^M^M                                                                                                               
         sig2fi=FoA0FiS2+FoA1FiS2*thr*r2d+FoA2FiS2*(thr*r2d)**2!theta in degrees^M^M                                                                                                                  
^M^M
         pout=p+ran(1)*p*sqrt((sig1p*p)**2+(sig2p/beta)**2)^M^M
         thout=thr+ran(2)*sqrt(sig1th**2+(sig2th/p/beta)**2)^M^M
         phiout=phir+ran(3)*sqrt(sig1fi**2+(sig2fi/p/beta)**2)^M^M
    */
    ...
    public double getSigmaPhi(Particle part, double torus){
        ...
        double beta   = part.p()/part.vector().e();
        double p      = part.p();
        double sig1fi=params.get("FoA0FiS1")/Math.toDegrees(part.theta()) +
                params.get("FoA1FiS1") + params.get("FoA2FiS1")* Math.toDegrees(part.theta());        
    ...
    public double getSigmaMom(Particle part, double torus){
        ...
        double p      = part.p();
        /*
        double sig1p=(1.0/torus)*
                (params.get("FoMT1S1")+params.get("FoMT1S2")*Math.toDegrees(part.theta())+
                params.get("FoMT1S3")*Math.toDegrees(part.theta())*Math.toDegrees(part.theta()));
         double sig2p=(1.0/torus)*
                 (params.get("FoMT2S1")+params.get("FoMT2S2")*Math.toDegrees(part.theta())+
                 params.get("FoMT2S3")*Math.toDegrees(part.theta())*Math.toDegrees(part.theta()));
                */
        double sig1p= params.get("FoInMoS1");
        double sig2p= params.get("FoInMoS2");
        
        result = part.p()*Math.sqrt(Math.pow(sig1p*part.p(),2)+Math.pow(sig2p/beta,2));
        return result;
    }
